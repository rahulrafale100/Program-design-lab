...
node list_insert_front(node head ,node value){
	node new_head;
	new_head=value;
	new_head->next=head;
	head=new_head;
...
node list_insert_tail(node head,node value){
	node new_node,t;
	new_node=value;
	if(head==NULL){
		head=new_node;
...
node list_delete(node head,node k){
...
int main(){
	...
	while(1){
		...
		if(c=='f'){
			scanf("%d",&k);
			ct=create_node(k);
			head_pro=list_insert_front(llist->head ,ct);
			llist->head=head_pro;
		}
		if(c=='t'){
			scanf("%d",&k);
			ct=create_node(k);
			head_pro=list_insert_tail(llist->head,ct);
			llist->head=head_pro;
		}
		if(c=='a'){
			scanf("%d%d",&k,&m);
		   ct=create_node(k);
		   d=list_search(llist->head,m);
			head_pro=list_insert_after(llist->head,ct,d);
			llist->head=head_pro;
		}
		if(c=='b'){
		scanf("%d%d",&k,&m);
		ct=create_node(k);
		d=list_search(llist->head,m);
		head_pro=list_insert_before(llist->head,ct,d);
		llist->head=head_pro;}
		if(c=='d'){
			scanf("%d",&k);
			...
			head_pro=list_delete(llist->head,ct);
			llist->head=head_pro;}
		}
		if(c=='i'){
